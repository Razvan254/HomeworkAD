...
int main() {
    srand(time(NULL)); // Initializing seed for rand() function

    // Generate random number of books
    int num_books = rand() % 100 + 1; // Random number between 1 and 100
    printf("Number of books: %d\n", num_books);

    // Memory allocation for the array of books
    Book *books = (Book *)malloc(num_books * sizeof(Book));

    // Generate random number of pages for each book and print them
    printf("Number of pages for each book:\n");
    for (int i = 0; i < num_books; i++) {
        books[i].num_pages = rand() % 99000001 + 1000000; // Random number between 1,000,000 and 100,000,000
        printf("Book %d: %d pages\n", i + 1, books[i].num_pages);
    }
